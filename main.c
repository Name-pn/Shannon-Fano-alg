...
#define MAX_CODE 100 // Максимальное число байтов в коде сжатого
// символа
#define MAX_CHARS 1000 // Максимальное число символов в сжимаемом сообщении
#define ASCII_SIZE 256 // Размер исходного алфавита
#define COMPRESS_END_CHAR '\n' // Символ конца распаковки
...
int main() {
    ...
    // На английском так как кодировка UTF8
    printf("Программа получает текст на английском, сжимает его алгоритмом Шеннона - Фано, выводит двоичный код полученной строки,\nзатем раскодирует текст и выводит полученный результат\n");
    int n_in_str;

    char string [1000] = {0};
    char to_string[1000] = {0};
    char from_to_string[1000] = {0};

    printf("Введите строку:\n");
    fgets(string, 1000, stdin);
    n_in_str = (int)strlen(string);

    symbol a[MAX_CHARS];
    int alp = init_struct_arr(a, string, n_in_str);

    printf("Составлен алфавит:\n");
    write_alp(a, alp);

    int bytes;
    compress((unsigned char *)string, n_in_str, (unsigned char *)to_string, a, alp, &bytes);

    printf("Размер исходной строки в байтах: %i.\nКод сжатого текста, состоящий из %i байт:\n", n_in_str, bytes);
    printf("Процент сжатия: %i%%\n", (int)(((long double)bytes)/n_in_str*100));
    print_var(to_string, bytes);

    unpack((unsigned char *)to_string, (unsigned char *)from_to_string, a, alp);
